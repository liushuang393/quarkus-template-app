...
public class AuthResource {
    ...
    @Inject
    com.example.service.AuditLogService auditLogService;
    ...
    @APIResponse(responseCode = "400", description = "バリデーションエラー",
                content = @Content(mediaType = "application/json", schema = @Schema(implementation = com.example.dto.ErrorResponse.class)))
    public Response register(@jakarta.validation.Valid RegisterRequest request) {
        User user = userService.register(request, headers);

        // 監査ログ記録
        auditLogService.logSuccess(user.getId(), user.getUsername(), "USER_REGISTER", "User", user.getId().toString());
    ...
    public Response login(@jakarta.validation.Valid LoginRequest request) {
        return userService.authenticate(request.username, request.password)
            .map(user -> {
                ...
                // 監査ログ記録（成功）
                auditLogService.logSuccess(user.getId(), user.getUsername(), "USER_LOGIN", "User", user.getId().toString());
